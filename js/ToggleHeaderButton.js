export default class ToogleHeaderButton{
	...
	constructor(header){
		...
		// Ajout des evenement
		this.openHeader.addEventListener('click', event => {event.preventDefault();this.showNavBar()});
		this.closeHeader.addEventListener('click', event => {event.preventDefault();this.hideNavBar()});
		window.addEventListener('resize', event=>{this.windowGrow()});
	}
